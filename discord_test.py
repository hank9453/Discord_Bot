...
class DB:
  mydb = None
  def __init__(self):
    self.mydb  = mysql.connector.connect( host="remotemysql.com",user="ntxGWN33MO",passwd="SRrsGxzUnK",database="ntxGWN33MO")
  def connect(self):
    self.mydb = mysql.connector.connect( host="remotemysql.com",user="ntxGWN33MO",passwd="SRrsGxzUnK",database="ntxGWN33MO")
  ...
  def query(self, sql):
    try:
      ...
      if err == 2013 :
        ...
        cursor.execute(sql)
    return cursor
mydb = mysql.connector.connect( host="remotemysql.com",user="ntxGWN33MO",passwd="SRrsGxzUnK",database="ntxGWN33MO")
...
async def 創立軍團(ctx):
     try :
        ...
        if len(result) != 0 :
            ...
            return 0
        mycursor.execute("CREATE TABLE %s_member_list (Discord_id VARCHAR(255),NickName  VARCHAR(255))"%str(ctx.guild.owner.id))
        await ctx.send("創建成功 👍")
     except :
        await ctx.send("創建失敗😨")
@bot.command()
async def 加入軍團(ctx ,name :str):
    ...
    if len(myresult) != 0 :
        ...
        return 0
    sql = "INSERT INTO %s_member_list"%str(ctx.guild.owner.id)+ "(Discord_id, NickName) VALUES (%s, %s)"
    val = (str(ctx.author.id), name)
    mycursor.execute(sql,val)
    mycursor.execute("CREATE TABLE %s_%sdata (Time DATETIME,Boss TINYINT,Damage INT , Compensation BOOLEAN)"%(str(ctx.guild.owner.id),str(ctx.author.id)))
    mydb.commit()
...
@bot.command()
async def 強制退出(ctx ,discordid: int):
    if ctx.guild.owner.id == ctx.author.id :
        ...
        if len(myresult) == 0 :
            await ctx.send("資料庫無此數據 退出失敗!😪")
            return 0
        sql = "DELETE  FROM %s_member_list WHERE Discord_id = '%s'"%(str(ctx.guild.owner.id),str(discordid))
        mycursor.execute(sql)
        sql = "DROP TABLE IF EXISTS %s_%sdata"%(str(ctx.guild.owner.id),str(discordid))
        mycursor.execute(sql)
        mydb.commit()
        await ctx.send("%退出成功😥" )
    else :
...
async def 個人資料(ctx):
    ...
    embed.add_field(name="Discord ID ", value="%s"%myresult[0], inline=False)
    embed.add_field(name="遊戲暱稱", value="%s"%myresult[1], inline=False)
    times = 0
...
async def 代填(ctx,tag : str,boss:int,damage : int,compesation :str = None):
    if boss <0 or boss > 6 :
        ...
        return 0
    id = re.sub('[<>@!]',"",tag)
    sql = "SHOW TABLES  LIKE '%s_%sdata'"%(str(ctx.guild.owner.id),str(id))
    mycursor.execute(sql)
    result = mycursor.fetchall()
    if result == None :
        ...
        return 0
    now = datetime.utcnow()+timedelta(hours=8)
    sql = "INSERT INTO %s_%sdata"%(str(ctx.guild.owner.id),str(id))+"(Time,Boss,Damage,Compensation)VALUES (%s,%s,%s,%s)"
    if compesation=="補償" :
        val = (str(now),boss,damage,True)
    else :
        val = (str(now),boss,damage,False)
...
@bot.command()
async def help(ctx):
    embed = discord.Embed(title="姆咪機器人", description="有幾個超棒的功能可以使用唷👍\n有參數必須在前加空格才能正常使用:", color=0xeee657)
...
bot.run('Njg3MTAzMTk1MjQ3NjczNDEw.Xmg44Q.Hb4y0gwjH595Z594OFVIEdUb59U')
